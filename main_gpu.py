...
lenet = lenet.to(device)#转移到gpu
#损失函数 转移gpu
loss_function = nn.CrossEntropyLoss()
loss_function = loss_function.to(device)#转移到gpu
#优化器
...
for epoch in range(Epoch):
    ...
    for data in train_Dataloader:
        #将数据读取出来输入网络，并计算损失函数
        imgs, targets = data
        imgs = imgs.to(device)#转移gpu
        targets = targets.to(device)#转移gpu
        output = lenet(imgs)
    ...
    #保存模型
    torch.save(lenet, "./model/lenet_gpu_{}.pth".format(epoch+1))
